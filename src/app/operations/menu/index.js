...
export default function MenuScreen() {
  ...
  const availableWidth =
    width - appSidebarW - listPadding - totalGap - scrollbarW;
  ...
  const filteredMenu = menuItems
    ...
    .map((p) => ({ ...p, onDelete: deleteMenuItem }));
  ...
  const sections = Object.keys(grouped)
    ...
    .map((category) => {
      ...
      return { title: category, data: rows, itemCount: items.length };
    });
  ...
  const handleToggleStatus = async (menuItem) => {
    try {
      ...
      const id = menuItem._id || menuItem.id;
      await dispatch(updateMenuItemStatus({ id, status: newStatus })).unwrap();
    } catch (error) {
  ...
  const handleSaveMenuItem = async (data) => {
    try {
      ...
      if (editingMenuItem) {
        ...
        await dispatch(updateMenuItemThunk({ id, payload })).unwrap();
      } else {
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 18, color: ThemeColors.textPrimary },
  sectionCount: {
    fontSize: 12,
  ...
  btnPrimaryText: { color: ThemeColors.white, fontSize: 14 },
  scrollContent: { padding: ThemeSpacing.md, paddingBottom: 100, flexGrow: 1 },
  cardWrapper: { flexShrink: 0, flexDirection: "column" },
  selectOverlay: {
    position: "absolute",
    top: 0,
    left: 0,
  ...
  bulkActionText: { color: ThemeColors.textPrimary, fontSize: 16 },
  fab: {
    position: "absolute",
